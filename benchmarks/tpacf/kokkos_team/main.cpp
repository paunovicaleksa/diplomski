...
int main( int argc, char **argv ) {
    Kokkos::initialize(argc, argv);
    {
        ...
        Kokkos::View<float*> binb("binb", nbins + 1);
       ...
       // memory for DD
        ...
        Kokkos::View<long long*> DD("DD", memsize);
            
        // memory for RR
        // std::vector<long long> RRS(memsize, 0);
        Kokkos::View<long long*> RRS("RRS", memsize);
            
        // memory for DR
        // std::vector<long long> DRS(memsize, 0);
        Kokkos::View<long long*> DRS("DRS", memsize);
            
        // memory for input data
        // std::vector<cartesian> data(args.npoints);
        // std::vector<cartesian> random (args.npoints);
        Kokkos::View<cartesian*> data("data", args.npoints);
        Kokkos::View<cartesian*> random("random", args.npoints);
        ...
        // read data file
        npd = readdatafile(args.data_name.c_str(), data, args.npoints);
        if (npd != args.npoints) {
            fprintf(stderr, 
        ...
        pb_SwitchToTimer(&timers, pb_TimerID_COMPUTE);
        // compute DD
        doCompute(data, npd, random, 0, 1, DD, nbins, binb);

        // loop through random data files
        for (rf = 0; rf < args.random_count; rf++) {
            // read random file
            pb_SwitchToTimer(&timers, pb_TimerID_IO);
            npr = readdatafile(args.random_names[rf].c_str(), random, args.npoints);
            if (npr!= args.npoints) {
                fprintf(stderr, 
                    "Error: read %i data points out of %i\n", 
                    npr, args.npoints
            ...
            pb_SwitchToTimer(&timers, pb_TimerID_COMPUTE);
            // compute RR
            doCompute(random, npr, random, 0, 1, RRS, nbins, binb);
