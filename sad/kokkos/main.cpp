...
int main ( int argc, char **argv ) {
    Kokkos::initialize(argc, argv);
    {
        ...
        image_i16 ref_image, cur_image;;
        if(!load_image ( in_file1.c_str(), ref_image)) {
            fprintf(stderr, "Erorr loading image %s\n", in_file1.c_str());
        }
        if(!load_image ( in_file2.c_str(), cur_image)) {
            fprintf(stderr, "Erorr loading image %s\n", in_file2.c_str());
        }
        ...
        Kokkos::View<short*> sads_computed("sads_computer", 41 * MAX_POS_PADDED * image_size_macroblocks);
        ...
        larger_sads ( sads_computed, image_size_macroblocks, 41 * MAX_POS_PADDED * image_size_macroblocks);
        ...
        if (out_file != "") {
            ...
            write_sads (
                ...
                ( unsigned short* ) (sads_computed.data())
            );
